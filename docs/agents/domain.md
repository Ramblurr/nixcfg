...
If any of these files don't exist, **proceed silently**. Don't flag their absence; don't suggest creating them upfront. Skill(domain-modeling) (reached via Skill(grill-with-docs) and Skill(improve-codebase-architecture)) creates them lazily when terms or decisions actually get resolved.
