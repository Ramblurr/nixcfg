...
let
  cfg = config.modules.hardware.easyNvidia.offload;
in
{
  options.modules.hardware.easyNvidia.offload = with lib.types; {
    enable = lib.mkOption {
      type = bool;
      default =
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.udev.packages =
      ...
      in
      lib.mkIf (config.modules.hardware.easyNvidia.desktopEnvironment != "x11") (
        lib.singleton (
