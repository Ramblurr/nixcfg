...
in {
  options.modules.desktop.services.ha-shutdown = {
    ...
    environmentFile = mkOption {
      description =
        "The full path to a file that contains the secret environment variables needed for the shutdown service";
    ...
    timeout = mkOption {
      ...
      default = 60000;
      description =
        "The number of milliseconds to wait for a response from Home Assistant before shutting down";
    };
  };
  config = mkIf cfg.enable {
    ...
    systemd.user.services.ha-shutdown = {
      ...
      serviceConfig = {
        ...
        ExecStart = "${pkgs.python3}/bin/python -u ${shutdownScript} --timeout ${
