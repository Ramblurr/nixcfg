...
in {
  options.modules.services.microsocks = { enable = mkBoolOpt false; };
  config = mkIf cfg.enable {
    environment.systemPackages = with pkgs; [ my.microsocks ];
    #services.microsocks = {
    #  enable = true;
    #  package = davispkgs.microsocks;
    #  port = 1081;
    #  # authUsername = "user";
    #  # authPasswordFile = /etc/microsocks-auth;
    ...
    systemd.services = {
      mullvad-exclusion-init = mkIf useMullvad {
        ...
        serviceConfig = {
          User = "root";
          ExecStart =
            "/bin/sh -c '${pkgs.coreutils}/bin/mkdir -p /sys/fs/cgroup/net_cls/mullvad-exclusions/ && ${pkgs.coreutils}/bin/touch /sys/fs/cgroup/net_cls/mullvad-exclusions/cgroup.procs && ${pkgs.coreutils}/bin/chmod 777 /sys/fs/cgroup/net_cls/mullvad-exclusions/cgroup.procs'";
          NotifyAccess = "all";
          Type = "oneshot";
          RemainAfterExit = "yes";
        };
      ...
      microsocks = {
        ...
        serviceConfig = {
          ...
          Restart = "on-failure";
          RestartSec = "10s";
          ExecStart = if useMullvad then
            "${pkgs.mullvad-vpn}/bin/mullvad-exclude ${pkgs.my.microsocks}/bin/microsocks -i 127.0.0.1 -p 1081"
          else
            "${pkgs.my.microsocks}/bin/microsocks -i 127.0.0.1 -p 1081";
        };
