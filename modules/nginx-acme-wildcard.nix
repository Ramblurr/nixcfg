# source https://github.com/oddlama/nix-config/blob/main/modules/acme-wildcard.nix
{ config, lib, ... }:
let inherit (lib) assertMsg filter genAttrs hasInfix head mkIf mkOption removeSuffix types;
in {
  ...
  options.services.nginx.virtualHosts = mkOption {
    type = types.attrsOf (types.submodule (submod: {
      options.useACMEWildcardHost = mkOption {
        ...
        default = false;
        description =
          "Automatically set useACMEHost with the correct wildcard domain for the virtualHosts's main domain.";
      };
      config = let
        ...
        matchingCerts = filter (x: !hasInfix "." (removeSuffix ".${x}" domain))
          config.security.acme.wildcardDomains;
      in mkIf submod.config.useACMEWildcardHost {
        useACMEHost = assert assertMsg (matchingCerts != [ ])
  ...
  config.security.acme.certs = genAttrs config.security.acme.wildcardDomains
