...
with lib; let
  ...
  toTomlKey = k:
    if builtins.match "[A-Za-z0-9_-]+" k != null
    then k
    else ''"${k}"'';
  ...
  toTomlValue = v:
    ...
    then ''[${concatMapStringsSep ", " toTomlValue v}]''
    else if isAttrs v
    then ''{ ${concatStringsSep ", " (mapAttrsToList toTomlKeyValue v)} }''
    ...
    categorized =
      mapAttrs (
        _: v:
          if isAttrs v
          then "attrs"
          else if isList v && all (v: isAttrs v) v
          then "list"
          else "value"
      )
      attrs;
    cat = cat:
      filterAttrs (_: v: v != null) (mapAttrs (k: v:
        if v == cat
        then attrs.${k}
        else null)
      categorized);
