...
- Agents may run OpenTofu/Terraform commands when a human-approved ticket, specification, or requested workflow explicitly requires them. That approval covers the commands needed by the stated workflow; do not ask again before each command. If the approved work does not explicitly require OpenTofu/Terraform execution, prepare the commands and ask before running them. Follow the operational-change protocol for state-changing operations.
