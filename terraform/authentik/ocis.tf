...
locals {
  ...
  home_ocis_name   = var.home_ocis_name
...
resource "authentik_provider_oauth2" "ocis-desktop" {
  ...
  invalidation_flow   = local.default_provider_invalidation_flow
  property_mappings   = concat(data.authentik_property_mapping_provider_scope.oauth2_offline.ids, data.authentik_property_mapping_provider_scope.oauth2.ids)
...
resource "authentik_provider_oauth2" "ocis-android" {
  ...
  property_mappings   = concat(data.authentik_property_mapping_provider_scope.oauth2_offline.ids, data.authentik_property_mapping_provider_scope.oauth2.ids)
  allowed_redirect_uris = [
    { matching_mode = "strict", url = "oc://android.owncloud.com" }
  ]
  lifecycle {
    ignore_changes = [
      signing_key
...
resource "authentik_provider_oauth2" "ocis-ios" {
  ...
  property_mappings   = concat(data.authentik_property_mapping_provider_scope.oauth2_offline.ids, data.authentik_property_mapping_provider_scope.oauth2.ids)
  allowed_redirect_uris = [
    { matching_mode = "strict", url = "oc://ios.owncloud.com" },
    { matching_mode = "strict", url = "oc.ios://ios.owncloud.com" }
  ]
  lifecycle {
    ignore_changes = [
      signing_key
