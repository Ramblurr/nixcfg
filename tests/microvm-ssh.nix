...
let
  ...
  guest =
    name:
    (inputs.self.lib.nixcfg.mkGuest "immich-home" {
      extraModules = [
        {
  ...
  checkGuest =
    ...
    let
      cfg = guest name;
    in
    cfg.microvm.vsock.cid == registry.dewey.guests.${name}.cid
  ...
  helper = builtins.head (
    builtins.filter (p: (p.name or "") == "microvm-verified-ssh") host.environment.systemPackages
  );
in
assert lib.assertMsg (lib.all checkGuest registered)
  "Both guests must use standard root authorized keys without a custom VSOCK override";
assert lib.assertMsg (
  !(builtins.elem registry.dewey.publicKeyFile (guest "unregistered")
  .users.users.root.openssh.authorizedKeys.keyFiles)
) "Unregistered guests must not acquire host access";
assert lib.assertMsg (lib.all (
  name:
  host.programs.ssh.knownHosts."microvm-${name}".publicKeyFile
  == registry.dewey.guests.${name}.hostKeyFile
) registered) "Guest host identities must be pinned";
